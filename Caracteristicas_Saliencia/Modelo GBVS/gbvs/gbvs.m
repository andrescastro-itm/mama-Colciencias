...
if ( strcmp(class(img),'uint16') == 1 ) img = double(img)/((2^16)-1); end%%%%%%%%%%%%%% añadida
...
for j=1:length(norm_maps)
  map = norm_maps{j}.map;
  fmapi = norm_maps{j}.maptype(1);%%%maptype [---] vector que marca el grupo o el canal de la caracteristica
  Nfmap{fmapi} = Nfmap{fmapi} + 1;
  cmaps{fmapi} = cmaps{fmapi} + map;%% CMAPs contiene la union de los submapas correspondientes a cada canal de caracter...
end
%%% divide each feature channel by number of maps in that channel
for fmapi = 1 : length(mapnames)
  if ( param.normalizeTopChannelMaps)    %%%%Ya se normalizo mapa por mapa en la seccion 3, o Modificar param.normalizeTopChannelMaps con un valor de 1 para sea normalizado por canales
...
master_map = attenuateBordersGBVS(master_map,4);  %%%%%%%%%%%%%%%%%% Mapa de Sliencia de 32x32 %%%%%%%%%%%%%
...
if ( blurfrac > 0 )
  ...
  master_map_filtrado = myconv2(myconv2( master_map , k ),k');%master_map = myconv2(myconv2( master_map , k ),k');
  master_map_filtrado = mat2gray(master_map_filtrado); %master_map = mat2gray(master_map);
end

if ( param.unCenterBias )       %%%%%%%%%%%%%%%%%%%%%%%%%%% REVISAR %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:length(mapnames)
conspicuity_map{i}=mat2gray(comb_norm_maps{i});
conspicuity = conspicuity_map{i}; 
conspicuity_map_resized{i} = mat2gray(imresize(conspicuity,[size(img,1) size(img,2)]));

% figure(i+3)
% imshow(conspicuity_map_resized{i})
% title('Conspicuity')
end
...
master_map_filtrado_resized = mat2gray(imresize(master_map_filtrado,[size(img,1) size(img,2)])); %Agregada

out = {};
out.master_map = master_map;
out.master_map_resized = master_map_resized;
out.master_map_filtrado_resized = master_map_filtrado_resized;
out.top_level_feat_maps = feat_maps;
out.Conspicuity_maps_resized = conspicuity_map_resized; %% añadida, para tener los mapas de Conspicuity intensity(1)-motion(2)-orientation(3), redimencionada
out.map_types = map_types;
out.intermed_maps = intermed_maps;   %%%%% se encuentran todos los mapas
