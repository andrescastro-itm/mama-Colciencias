...
function [Roi_MRI Roi_Estudios] = lecturEstudio(phad,savepaht,N,z,w,h)
...
for i = vect
   ...
   tipos={'*DIF*'};
   tipos = [tipos;'*MAPA*';'*T1*';'*T2*';'*ST_COR*';'*T2W_SPAIR*';'*sFase*';'*DIN*'];
...
%----------------------------- Condicion para Leer los CSV.
            ...
            elseif (strncmp(tip,'ST_COR',2)|strncmp(tip,'T2W_SPAIR',3))%(strncmp(tip,'ST_COR',2)|strncmp(tip,'eST_COR',3)|strncmp(tip,'sT2W_SPAIR',4)|strncmp(tip,'T2W_SPAIR',3))%((length(tip) == length('ST_COR'))&(tip == 'ST_COR'))|((tip == length('T2W_SPAIR'))&(tip == 'T2W_SPAIR'))
        ...
        for r = 1:size(M,1)%length(Rname)  % punto                           %%%%%%%%%% unicia el recorte de la imagen
         ...
         if ((TIPO == S)&(~isempty(pathempty)))%((isempty(busc)) & (tip == parts.secuencia(1:length(tip))))%tip == parts.secuencia
             if parts.Partida=='I'
                ...
                %for y = 1:size(M,1)
                if ((length([parts.letters,'_','F'])==length(M{y,8}))&([parts.letters,'_','F']==M{y,8}))

                     x_anF = M{y,24}; %primer puntp (esquina superior izquierda)
                     y_anF = M{y,25};
                ...
                coorP1 = [x_anF y_anF; x_anI y_anI];
                
                coorP3 = [x_posF y_posF; x_posI y_posI];
                
             elseif parts.Partida=='F'
                 continue %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                 x_anF = M{r,24}; %primer puntp (esquina superior izquierda)
                 y_anF = M{r,25};
                ...
                %for y = 1:size(M,1)
                if ((length([parts.letters,'_','I'])==length(M{y,8}))&([parts.letters,'_','I']==M{y,8}))

                     x_anI = M{y,24}; %primer puntp (esquina superior izquierda)
                     y_anI = M{y,25};
                ...
                coorP1 = min([x_anF y_anF; x_anI y_anI]);
                coorP3 = max([x_posF y_posF; x_posI y_posI]);
                
             elseif parts.Partida=='S'
                 ...
                 y_posI = abs(M{r,25}-M{r,25+10});%M{r,25+10};
                 roiI = [x_anS y_anS x_posS y_posS];
                 
                 zindI = r; %r+1;
                 zindF = zindI;
                 
              elseif parts.Partida=='C' % punto
